            ...
            for (var i = 0; i < this._owner.groups.length; i++) {
                ...
                if (group.properties) {
                    for (var i = 0; i < group.properties.length; i++) {
                        var entityProperty = group.properties[i];
                        this._owner._attachPropertyChangeListener(entityProperty);
                        entityProperty._updateNativeEditor(entityProperty.editor.ios);
                    }
                }
...
var RadDataForm = (function (_super) {
    ...
    RadDataForm.getFontWithProperties = function (fontName, size, style) {
        ...
        if (style) {
            var traits = 0 /* UIFontDescriptorClassUnknown */;
            switch (commonModule.FontStyles[style]) {
                case commonModule.FontStyles.Bold:
                    traits = 1 /* UIFontDescriptorTraitBold */;
                    break;
                case commonModule.FontStyles.Italic:
                    traits = 2 /* UIFontDescriptorTraitItalic */;
                    break;
                case commonModule.FontStyles.BoldItalic:
                    traits = 1 /* UIFontDescriptorTraitBold */ | 2 /* UIFontDescriptorTraitItalic */;
                    break;
            }
            if (!font) {
                font = UIFont.systemFontOfSize(fontSize);
...
var EntityProperty = (function (_super) {
    ...
    EntityProperty.prototype.updateNativeValuesProvider = function (value) {
        ...
        for (var i = 0; i < value.length; i++) {
            nativeSource.addObject(value[i].trim());
        }
...
var PropertyEditor = (function (_super) {
    ...
    PropertyEditor.prototype._applyParams = function () {
        ...
        if (editorParams.minimum && editorParams.maximum) {
            if (!isNaN(editorParams.minimum) && !isNaN(editorParams.maximum)) {
                this._updateNativeRange(TKRange.rangeWithMinimumAndMaximum(editorParams.minimum, editorParams.maximum));
            }
        }
        if (editorParams.step && !isNaN(editorParams.step)) {
            this._updateNativeStep(editorParams.step);
        }
    };
    PropertyEditor.prototype._updateNativeRange = function (range) {
        if (this._editorRange == range) {
            return;
        }
        this._editorRange = range;
        if (this.ios) {
            this.ios.property.range = this._editorRange;
            if (this.ios.editor instanceof UIStepper) {
                this.ios.editor.minimumValue = this.ios.property.range.minimum;
                this.ios.editor.maximumValue = this.ios.property.range.maximum;
            }
            else if (this.ios.editor instanceof UISlider) {
                this.ios.editor.minimumValue = this.ios.property.range.minimum;
                this.ios.editor.maximumValue = this.ios.property.range.maximum;
            }
        }
    };
    PropertyEditor.prototype._updateNativeStep = function (step) {
        if (this._editorStep == step) {
            return;
        }
        this._editorStep = step;
        if (this.ios) {
            this.ios.property.step = this._editorStep;
            if (this.ios.editor instanceof UIStepper) {
                this.ios.editor.stepValue = this.ios.property.step;
            }
        }
    };
